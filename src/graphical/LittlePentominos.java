...
public class LittlePentominos extends JPanel{
    ...
    public MosaicPanel board;  // for displaying the board on the screen
    ...
    public LittlePentominos() {
        ...
        board = new MosaicPanel(8,8,50,50,Color.GRAY,2);  // for displaying the board
        board.setAlwaysDrawGrouting(true);
        board.setDefaultColor(Color.WHITE);
        board.setGroutingColor(Color.LIGHT_GRAY);

        board.addMouseListener(new PentominosMouseAdapter(this));

        setLayout(new BorderLayout());
        add(board, BorderLayout.CENTER);

        this.game = new PentominosBoard();
